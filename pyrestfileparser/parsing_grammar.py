...
COMMENT_LINE = LineStart() + Optional(White(" \t")) + (Literal("#") | Literal("//")) + restOfLine + LineEnd()
...
def request_line_definition():
    ...
    return (LineStart() + Optional(method + White()) + url + Optional(White() + http_version) + LineEnd())(
        "request_line"
    )
...
def request_block_definition():
    ...
    bdy = body_definition()
    return Group(ZeroOrMore(COMMENT_LINE) + req_line + hdrs + Optional(bdy) + Optional(DELIMITER))("request")
